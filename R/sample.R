CSample <- R6::R6Class(
	...
	f_mean = NULL, geom = NULL, var = NULL, s_cov = NULL
    ),
    public = list(
        initialize = function(conns = NULL, tan_imgs = NULL, 
				vec_imgs = NULL, centered = NULL, geom) {
            if (is.null(geom)) stop("geom must be specified.")
            ...
            if (!is.null(conns)) {
                ...
                n <- length(conns); p <- nrow(conns[[1]]); d <- p * (p + 1) / 2
                        ...
                        private$geom <- geom; private$var <- NULL; private$s_cov <- NULL
                ...
                private$f_mean <- frechet_mean; private$geom <- geom; private$var <- NULL
                private$s_cov <- NULL
            } else {
                if (is.null(vec_imgs)) {
                ...
                private$f_mean <- frechet_mean; private$geom <- geom; 
        ...
        compute_unvecs = function() {
            ...
            purrr::map(\(i) self$geometry$unvec(private$vec_imgs[[1]], 
        ...
        compute_fmean = function(tol = 0.05, max_iter = 20, lr = 0.2) {
            ...
            while ((delta > tol) && (iter < max_iter)) {
                ...
                new_ref_pt <- self$geometry$exp(old_ref_pt, tan_step)
                ...
                aux_sample <- CSample$new(
                        ...
                        centered = FALSE, geom = self$geometry)
            }
        ...
        change_ref_pt = function(new_ref_pt) {
            ...
            new_tan_imgs <- relocate(private$tan_imgs[[1]], new_ref_pt, 
                private$tan_imgs[[2]], self$geometry)
        ...
        compute_sample_cov = function() {
            ...
            private$s_cov <- stats::cov(private$vec_imgs[[2]])
        }
    ),
    active = list(
